...
class ArticulosSheet implements FromCollection, WithHeadings, WithMapping, WithStyles, WithColumnWidths, WithTitle
{
    ...
    {
        // Solo artículos
        $this->publications = $publications->filter(fn($p) => $p->article !== null)->values();
    }

    public function title(): string
    {
        return 'Artículos Científicos';
    }
    ...
    public function map($publication): array
    {
        ...
        return [
            $this->rowNumber,
            $publication->title,
            $publication->article->journal?->journal_name ?? '—',
            $publication->article->journal_issn ?? '—',
            $publication->article->journal?->category ?? '—',
